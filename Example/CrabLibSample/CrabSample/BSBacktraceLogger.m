...
+ (NSString *)bs_backtraceOfNSThread:(NSThread *)thread {
    ...
    for(NSString * keyValuePair in keyValuePairs) {
        ...
        range = [key rangeOfString:@"number"];
        if (range.location != NSNotFound)  {
            idx = [components[1] integerValue];
        }
    }
...
//        NSLog(@"frame=%lX", backtraceBuffer[i]); // 首地址
//        [resultString appendFormat:@"%@", bs_logBacktraceEntry(i, backtraceBuffer[i], &symbolicated[i])];
        [resultString appendFormat:@"%@", bs_logBacktraceAddress(i, backtraceBuffer[i])];
    }
...
NSString* bs_logBacktraceAddress(const int entryNum,
    ...
    //    const uintptr_t addressWithSlide = address - imageVMAddrSlide;
        ...
        //    NSLog(@"%X", segmentBase);
        // 获取内存中某个模块的名称。
        const char *fname = _dyld_get_image_name(idx);
    
        //Get name of file
        long slength = strlen(fname);
    
        long j;
        for(j = slength - 1; j>= 0; --j)
            if(fname[j] == '/') break;
        char *name = strndup(fname + (++j), slength - j);
    
        void *fbase = (void*)header;
        uintptr_t offset = address - (uintptr_t)fbase;
    
        return [NSString stringWithFormat:@"%-4ld%-30s  0x%08" PRIxPTR " 0x%lx + %lu\n" ,(long)entryNum, name, address, (uintptr_t)fbase, offset];
...
//    NSLog(@"%lX--%lX--%lX", address, (uintptr_t)dlInfo->dli_fbase, (uintptr_t)dlInfo->dli_saddr);
...
//    NSLog(@"%s", sname);
    ...
    return [NSString stringWithFormat:@"%-4ld%-30s  0x%08" PRIxPTR " %s + %lu\n" ,(long)entryNum, fname, (uintptr_t)address, sname, offset];
}
